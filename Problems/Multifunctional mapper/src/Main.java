...
class MultifunctionalMapper {
    ...
    public static final Function<List<IntUnaryOperator>,
            UnaryOperator<List<Integer>>> MULTIFUNCTIONAL_MAPPER = operatorsList->{
                return (List<Integer> intList)->{
                    return intList.stream().map(intValue-> {
                        int result = intValue;
                        for (IntUnaryOperator operator : operatorsList) {
    ...
    public static final UnaryOperator<List<Integer>>
            SQUARE_AND_THEN_GET_NEXT_EVEN_NUMBER_TRANSFORMATION =
